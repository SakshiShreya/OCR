...
def findLines(bw_image, linesThres=0):
	# blurring lines to sepearate them
	bw_image = cv2.GaussianBlur(bw_image,(99,25),0)
	bw_image *= 2
	ret, bw_image = cv2.threshold(bw_image,0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
